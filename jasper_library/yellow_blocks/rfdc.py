...
class rfdc(YellowBlock):
  ...
  def initialize(self):
    ...
    m = re.search('(2[8-9])|(39)|(4[7-9])', part)#lijian
    if not m:
  ...
  def modify_top(self, top):
    ...
    if self.enable_mts_adc or self.enable_mts_dac:
      ...
      #orig mts_inst.add_port('pl_clk_buf', 'pl_clk_buf')
      mts_inst.add_port('pl_clk_buf', 'user_clk')#20231030
    ...
    for tidx in self.enabled_dac_tiles:
      ...
      for didx in self.enabled_dacs:
        ...
        if int(didx[0]) == tidx: # need this becuase of enabled_dacs stores ALL enabled slices across all dac tiles, we only want the adcs associated with this tile
          if(self.gen == 3 and self.num_dac_slice == 2):
            d = self.dacs[int(didx[1])+2*int(didx[0])]
          else:
          ...
          if self.tile_arch == 'QT':
            ...
            if(self.gen == 3 and self.num_dac_slice == 2):#20231021
              n_didx = int(didx[1])*2
            else:
              n_didx = int(didx[1])           
            bd_inst.add_port('vout{:d}{:d}_p'.format(tidx, n_didx), 'vout{:d}{:d}_p'.format(tidx, n_didx),  dir='out', parent_port=True)
            bd_inst.add_port('vout{:d}{:d}_n'.format(tidx, n_didx), 'vout{:d}{:d}_n'.format(tidx, n_didx),  dir='out', parent_port=True)
  ...
  def gen_tcl_cmds(self):
    ...
    for tidx in self.enabled_dac_tiles:
      ...
      for didx in self.enabled_dacs:#20231021
        if int(didx[0]) == tidx:
          n_didx = int(didx[1])
          if(self.gen == 3 and self.num_dac_slice == 2):
            d = self.dacs[int(didx[1])+2*int(didx[0])]
            tcl_cmds['pre_synth'] += self.build_config_cmd(d, self.dac_attr_map, tidx, n_didx*2)
          else:
            d = self.dacs[int(didx[1])+4*int(didx[0])]
            tcl_cmds['pre_synth'] += self.build_config_cmd(d, self.dac_attr_map, tidx, n_didx)
