function adc_data_transform_mask(gcb)
...
for i =1:length(gateway_outs)
    ...
    if regexp(gw_name, 'data_valid_in$')
        set_param(gw, 'Name', clear_name([gcb, '_data_valid_in']));
    elseif regexp(gw_name, 'data_in1$')
        set_param(gw, 'Name', clear_name([gcb, '_data_in1']));
    elseif regexp(gw_name, 'data_in2$')
        set_param(gw, 'Name', clear_name([gcb, '_data_in2']));        
    else 
...
for i =1:length(gateway_ins)
    ...
    if regexp(gw_name, 'clk_out$')
        set_param(gw, 'Name', clear_name([gcb, '_clk_out']));
    elseif regexp(gw_name, 'data_valid_out$')
        set_param(gw, 'Name', clear_name([gcb, '_data_valid_out']));   
    elseif regexp(gw_name, 'data_out1$')
        set_param(gw, 'Name', clear_name([gcb, '_data_out1'])); 
    elseif regexp(gw_name, 'data_out2$')
        set_param(gw, 'Name', clear_name([gcb, '_data_out2']));         
    else 
        parent_name = get_param(gw, 'Parent');
        errordlg(['Unknown gateway: ', parent_name, '/', gw_name]);
    end
